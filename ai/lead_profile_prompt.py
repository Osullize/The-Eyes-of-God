...
DEFAULT_PROMPT_VERSION = "heat_pump_lead_cn_v2"
...
联系方式分析规则：
- 联系方式位于 profile_package.contacts。除了 emails、phones、social_links、people，还可能包含由数据库当前联系方式表聚合出的 normalized_records；
- 先按联系方式类型和值去重，再判断哪些渠道可直接用于销售触达；
- 优先推荐有明确姓名和职位的采购、进口、销售、渠道、负责人或管理层联系人，其次是销售/商务邮箱，再其次是公司电话和社交媒体；
- recommended_contacts 最多返回 3 条，value 必须逐字来自输入，禁止编造、补全、改写或猜测邮箱、电话、姓名和社媒地址；
- contactability 评分必须与 contact_analysis 一致：有具名业务联系人和直接邮箱/电话可给 9-10 分；有有效公司邮箱或电话可给 6-8 分；只有社媒或弱联系入口给 3-5 分；没有可用联系方式给 0-2 分；
- 如果没有可用联系方式，contact_quality 使用 none，available_channels 和 recommended_contacts 返回空数组，preferred_channel 使用 none，并在 outreach_strategy 中说明需要人工补充联系方式；
- contact_analysis 中的 reason 和 outreach_strategy 必须使用中文。
...
所有自然语言内容必须使用中文，包括 profile_summary、business_type、market_role、product_fit、evidence、risk_flags、recommended_action、contact_analysis.reason 和 contact_analysis.outreach_strategy。company_name 和输入中的联系方式值可以保留原文。JSON 字段名必须保持英文。

JSON 对象必须包含：
company_name, profile_summary, business_type, market_role, product_fit, customer_priority,
score_total, score_breakdown, contact_analysis, evidence, risk_flags, recommended_action。
...
RESULT_JSON_SCHEMA: dict[str, object] = {
    ...
    "properties": {
        ...
        "contact_analysis": {
            "type": "object",
            "properties": {
                "contact_quality": {"type": "string", "enum": ["high", "medium", "low", "none"]},
                "available_channels": {
                    "type": "array",
                    "items": {
                        "type": "string",
                        "enum": ["email", "phone", "social", "person", "contact_form"],
                    },
                },
                "preferred_channel": {
                    "type": "string",
                    "enum": ["email", "phone", "social", "person", "contact_form", "none"],
                },
                "recommended_contacts": {
                    ...
                    "items": {
                        ...
                        "required": ["type", "value", "label", "reason"],
